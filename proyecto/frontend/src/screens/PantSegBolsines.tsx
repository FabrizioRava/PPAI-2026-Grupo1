...
import { glassPanel, PageBackground } from '../components/PageBackground';
...
export const PantSegBolsines: React.FC<PantSegBolsinesProps> = ({ usuario, onVolver }) => {
  ...
  const cmDestinosDisponibles = Array.from(new Set(bolsinesLocalizados.map((b) => b.cmDestinoNombre))).sort((a, b) =>
    a.localeCompare(b)
  );
  ...
  const bolsinesFiltrados = bolsinesLocalizados.filter((b) => {
    ...
    const coincidePrecinto = precintoStr.includes(busqueda) || codigoFormateado.toLowerCase().includes(busqueda);
    ...
    <div className="relative min-h-dvh flex flex-col font-sans antialiased text-brand-bgMain selection:bg-brand-primary selection:text-white overflow-hidden">
      ...
      {toast && (
        <div
          role="status"
          aria-live="polite"
          className={`fixed bottom-5 right-5 px-5 py-3.5 rounded-2xl backdrop-blur-xl border shadow-2xl transition-all duration-300 z-[100] flex items-center gap-3 max-w-md ${toast.type === 'success' ? 'bg-emerald-50/80 border-emerald-200/80 text-emerald-900' : 'bg-rose-50/80 border-rose-200/80 text-rose-900'}`}
        >
          <div
            className={`w-6 h-6 shrink-0 rounded-full flex items-center justify-center ${toast.type === 'success' ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'}`}
          >
            {toast.type === 'success' ? (
          ...
          <div className="flex items-center gap-3">
            {onVolver && (
              <button
                ...
                className="w-9 h-9 -ml-1 shrink-0 rounded-xl flex items-center justify-center text-brand-bgMain hover:bg-brand-bgMain/10 transition-colors focus:outline-none"
              >
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8 flex flex-col gap-6">
        <section className={`${glassPanel} rounded-2xl p-5 flex flex-col gap-4`}>
          <div className="flex flex-col md:flex-row gap-4">
            <div className="w-full md:w-1/2 flex flex-col gap-1.5">
              ...
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center text-brand-bgMain/50">
                ...
                <input
                  id="search-input"
                  type="text"
                  placeholder="Ingresa número de precinto (ej. 4501)"
                  value={filtroNumeroPrecinto}
                  onChange={(e) => setFiltroNumeroPrecinto(e.target.value)}
                  className="w-full bg-white/70 border border-white/70 rounded-xl py-2.5 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-brand-secondary/40"
                />
              </div>
            </div>
            ...
            <div className="w-full md:w-1/2 flex flex-col gap-1.5">
              ...
              <div className="relative">
                ...
                <select
                  ...
                  className="w-full appearance-none bg-white/70 border border-white/70 rounded-xl py-2.5 pl-10 pr-9 text-sm focus:outline-none focus:ring-2 focus:ring-brand-secondary/40 cursor-pointer"
                >
          ...
          <div className="flex items-center justify-end gap-3 text-xs text-brand-bgMain">
            <span>
              Encontrados:{' '}
              <strong className="text-white bg-brand-bgMain px-2.5 py-0.5 rounded font-bold">
                {bolsinesFiltrados.length}
              </strong>{' '}
              / {bolsinesLocalizados.length}
            </span>
            {hayFiltroActivo && (
              <button onClick={limpiarFiltros} className="text-brand-primary font-bold underline">
                Limpiar filtros
              </button>
            )}
          </div>
        ...
        {loading ? (
          ...
          <div className={`${glassPanel} rounded-2xl p-6 text-center max-w-md mx-auto my-12`}>
            ...
            <button
              ...
              className="mt-4 px-4 py-2 bg-brand-primary text-white rounded-lg text-xs font-bold"
            >
              Reintentar
            </button>
          </div>
          ...
          <div className={`${glassPanel} rounded-2xl p-8 text-center max-w-md mx-auto my-12`}>
            ...
            <p className="text-xs text-brand-bgMain/70 mt-2">
              No se encontraron bolsines en estado <strong>Enviado</strong> para la Comisión Médica{' '}
              <strong>{nombreCM}</strong>.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1">
            <div
              className={`lg:col-span-2 ${glassPanel} rounded-2xl overflow-hidden flex flex-col relative min-h-[420px]`}
            >
              ...
              <div className={`${glassPanel} rounded-2xl p-5 flex flex-col min-h-[240px] justify-between`}>
                {bolsinSeleccionado ? (
                  <div className="flex flex-col h-full justify-between">
                    <div>
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs font-bold text-brand-bgMain">
                          ID: #{bolsinSeleccionado.numeroPrecinto}
                        </span>
                      </div>
                      ...
                      <div className="mt-4 space-y-2 text-xs">
                        ...
                        <div className="flex justify-between items-center py-1.5">
                          ...
                          <span className="text-brand-bgMain font-mono font-bold">
                    ...
                    <div className="mt-5 flex flex-col gap-2">
                      <button
                        ...
                        className="w-full py-2.5 bg-brand-primary text-white font-bold rounded-xl text-xs shadow-md"
                      >
                        Notificar Ubicación
                      </button>
                    </div>
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-center py-8">
                    ...
                    <p className="text-xs text-brand-bgMain/70 mt-1 max-w-[200px]">
                      Haz clic sobre un pin del mapa para ver los datos.
                    </p>
                  </div>
                )}
              </div>
                    ...
                    bolsinesFiltrados.map((bolsin) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() => setBolsinSeleccionado(bolsin)}
                          className={`w-full p-3.5 text-left flex items-center justify-between outline-none ${seleccionado ? 'bg-brand-bgMain text-white border-l-4 border-brand-primary pl-2.5' : 'hover:bg-white/40 text-brand-bgMain'}`}
                        >
                          <div>
                            <span className="font-bold text-xs">
                              BOL-{String(bolsin.numeroPrecinto).padStart(3, '0')}
                            </span>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className={`${glassPanel} w-full max-w-sm rounded-2xl p-6 shadow-2xl`}>
            ...
            <p className="text-xs text-brand-bgMain/70 mt-2 leading-relaxed">
              Se registrará la ubicación actual del bolsín y se enviará una alerta automática al Gerente de la Comisión
              Médica Destino.
            </p>
            <div className="mt-5 flex items-center justify-end gap-3">
              <button
                ...
                className="px-3.5 py-2 bg-brand-bgMain/10 text-brand-bgMain rounded-xl text-xs font-bold hover:bg-brand-bgMain/20"
              >
                Cancelar
              </button>
              <button
                ...
                disabled={sendingMail}
                className="px-4 py-2 bg-brand-primary text-white rounded-xl text-xs font-bold hover:bg-brand-primary/90 flex items-center gap-1.5 shadow-sm"
              >
